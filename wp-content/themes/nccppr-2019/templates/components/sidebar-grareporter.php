<div class="callout accordion" id="accordion-years" role="tablist" aria-multiselectable="true">
  ...
  $days = $wpdb->get_results("SELECT DISTINCT DAY (post_date) AS day, MONTH( post_date ) AS month ,	YEAR( post_date ) AS year, COUNT( id ) as post_count FROM $wpdb->posts WHERE post_status = 'publish' and post_date <= now( ) and post_type = 'grareporter' GROUP BY day, month , year ORDER BY post_date DESC");
  ...
  foreach($days as $day) :
    ...
    <li class="archive-day">
      ...
      if ($grareporter->have_posts()) : while ($grareporter->have_posts()) : $grareporter->the_post(); ?>
