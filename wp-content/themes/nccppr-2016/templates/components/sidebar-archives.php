<div class="callout accordion" id="accordion-years" role="tablist" aria-multiselectable="true">
  ...
  $days = $wpdb->get_results("SELECT DISTINCT DAY (post_date) AS day, MONTH( post_date ) AS month ,	YEAR( post_date ) AS year, COUNT( id ) as post_count FROM $wpdb->posts WHERE post_status = 'publish' and post_date <= now( ) and (post_type = 'post' OR post_type = 'fridayfive' OR post_type = 'map' OR post_type = 'edtalk' OR post_type = 'flash-cards') GROUP BY day, month , year ORDER BY post_date DESC");
  ...
  foreach($days as $day) :
    ...
    <?php if ($year_current != $year_prev) { ?>
      ...
      <div class="panel archive-year">
        ...
        <div class="<?php if ($day->year == $expanded_year) { echo 'in'; } ?> wrapper-year" id="collapse-<?php echo $day->year; ?>" role="tabpanel" aria-labelledby="heading-<?php echo $day->year; ?>">
